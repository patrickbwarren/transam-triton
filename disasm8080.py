...
#
# Minor modifications are copyright (c) 2021 Patrick B Warren
...
parser.add_argument("-a", "--address", help="Specify starting address (defaults to 0)", default='0')
parser.add_argument("-s", "--skip", help="Skip initial 'n' bytes (defaults to 0)", default=0)
parser.add_argument("-b", "--binary", help="intepret all bytes as binary", action='store_true')
parser.add_argument("-f", "--format",
                    help="Use number format: 1=$1234 2=1234h 3=1234 4=177777 (default 1)",
                    default=1, type=int, choices=range(1, 5))
args = parser.parse_args()

# Get filename from command line arguments ('-' reads from stdin)

file_name = "/dev/stdin" if args.binary == "-" else args.binary
...
# Open input file.
# Display error and exit if file name does not exist.
...
while True:
    try:
        ...
        # Handle any operands
        if n == 2:
            if not args.binary and isprint(chr(op1)):
                line += "'%c'" % op1
            else:
                if args.format == 1:
